...
export default function Create({ accounts = [], invoices = [], clients = [], leads = [], paymentMethods = {}, statuses = {} }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    post(route('finance.payments.store'), {
      onSuccess: () => {
        toast.success(t('finance.payment_recorded', 'Payment recorded successfully'));
      },
      onError: () => {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {t('finance.record_payment_description', 'Record a new payment transaction')}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button type="submit" disabled={processing}>
              {processing ? t('common.saving', 'Saving...') : t('finance.record_payment', 'Record Payment')}
            </Button>
          </div>
        </form>
      </div>
