...
export default function Edit({ quotation, leads, currencies, statuses }: Props) {
  ...
  const { data, setData, put, processing, errors } = useForm({
    ...
    tax_rate: quotation.tax_amount > 0 ? ((quotation.tax_amount / quotation.subtotal) * 100) : 0,
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">
              {t('common.edit', 'Edit')} {t('finance.quotation', 'Quotation')} {quotation.quotation_number}
            </h1>
            <p className="text-muted-foreground">
              {t('finance.edit_quotation_description', 'Update quotation details and items')}
            </p>
          </div>
        </div>
