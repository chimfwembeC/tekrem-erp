...
export default function TaskEdit({ auth, project, task, milestones, staffUsers, availableTasks, tags }: TaskEditProps) {
  ...
  const { data, setData, put, processing, errors } = useForm({
    ...
    milestone_id: task.milestone_id?.toString() || 'empty',
    assigned_to: task.assigned_to?.toString() || 'empty',
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {availableTasks.map((availableTask) => (
                        <div key={availableTask.id} className="flex items-center space-x-2">
                          <Checkbox
                            id={`task-${availableTask.id}`}
                            checked={data.dependencies.includes(availableTask.id)}
                            onCheckedChange={() => handleDependencyToggle(availableTask.id)}
                          />
                          <Label htmlFor={`task-${availableTask.id}`} className="text-sm">
                            {availableTask.title}
                          </Label>
                        </div>
                      ))}
